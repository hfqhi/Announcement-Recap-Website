...
* **Lazy Auto-Archive Engine (New):** A lightweight background routine that silently sweeps the database on every page load, automatically migrating expired tasks to the archive without requiring manual cron jobs.
...
* **Mutable Subjects & Schedule Tracking:** Administrators can now edit existing subject codes and attach specific class time schedules (e.g., "M 10:00 AM - 1:00 PM") that instantly reflect on the public UI.
...
* **SQL Injection Prevention:** 100% Prepared Statements (PHP Data Objects) utilizing strict positional parameters (`?`) are strictly used across the entire system, alongside a strict structural whitelist for dynamic database table targeting.
